  ...
  while( (rec=data.pdom->NextFlippingItem()) != NULL){
	  ...
	  for (int i=0;i<plan->Length();i++){
		...
		if (closed) {
		  ...
		  CAction *ref_act_2 = (*plan)[i];
		  if (no_im_args){
		     
		     for (vector<int>::iterator it_in=++in_ma.begin();it_in!=in_ma.end();it_in++){
		           vector<sh_arg_str> *sh_args=new vector<sh_arg_str>();
		           ref_act_2->DetectSharedArgs((*plan)[*it_in],*sh_args);
		           ref_act_2 = new CMacroAction(ref_act_2,(*plan)[*it_in],*sh_args);
		  ...
		  if (!acc) continue; 
		  if (no_im_args && ref_act_2->GetParams()->Count()<ref_act->GetParams()->Count()) continue;
		  ...
		  for (int k=0;k<ref_act->GetPosEff()->Count();k++){
			...
			for (int q = 0;q<ref_act->GetPrec()->Count();q++){
			  if (strcasecmp((*ref_act->GetPosEff())[k]->GetName().data(),(*ref_act->GetPrec())[q]->GetName().data())==0 && ref_act->GetNegEff()->Find((*ref_act->GetPrec())[q])!=-1){
			    ...
			    if (!x.empty() && (*it_prob)->GetInit()->Find((*ref_act->GetPrec())[q])!=-1){
  ...
  while (mit!=sugg_macros.end()){
     ...
     for (vector<pair<CAction*,int> >::iterator mit2=sugg_macros.begin();mit2!=sugg_macros.end();mit2++){
	   ...
	   if (mit->first->GetPrec()->Count() < mit2->first->GetPrec()->Count() && mit->second<=mit2->second){
	     mit=sugg_macros.erase(mit);continue;
	  }
     }
     mit++;
  }
...
//  }
  ...
  for (mit=sugg_macros.begin();mit!=sugg_macros.end();mit++){
...
void CLearner::LearnMacrosFromAbsorbtions()
{
   ...
   for (deque<CProblem*>::iterator it_prob=data.train->begin();it_prob!=data.train->end();it_prob++){
	  ...
	  for (int it=0;it<plan->Length();it++){
		...
		if (((*(*plan)[ref_it]->GetPosEff())-(*(*plan)[ref_it]->GetPrec())).SubSetOf((*plan)[it2]->GetPrec()) && 
		   ...
		   if (found){ 
			  ...
			  vector<sh_arg_str> *shared_args = new vector<sh_arg_str>();
			  
			  ref_act->DetectSharedArgs((*plan)[it2],*shared_args);
			  	  
			  ref_act = new CMacroAction(ref_act, (*plan)[it2], *shared_args);
			  
			  data.pdom->GetActions()->FindProperAction((*plan)[it2]->GetActName(),s);
			  
			  macro = new CMacroAction(macro,(*data.pdom->GetActions())[s],*shared_args, data.pdom->GetTypes());
   ...
   for (vector<pair<CAction*,int> >::iterator mit=sugg_macros.begin();mit!=sugg_macros.end();mit++){
    ...
    for (vector<pair<CAction*,int> >::iterator mit2=sugg_macros.begin();mit2!=mit;mit2++){
	  ...
	  if (mit->first->GetParams()->Count() < mit2->first->GetParams()->Count() || 
	      mit->first->GetPrec()->Count() > mit2->first->GetPrec()->Count()){
	   //if (mit->second >= 2 * mit2->second) {mit2=sugg_macros.erase(mit2);mit2--;} else {mit=sugg_macros.erase(mit);mit--;break;}
   ...
   //frequencies of basic actions (or previously generated macros)
   for (int q=0;q<data.pdom->GetActions()->Count();q++){
     cout << *(this->frequency+q) << "x - " << (*data.pdom->GetActions())[q]->GetActName() <<endl;
